...
export const PROJECTS = [
  ...
  },
  {
    ...
    status: 'wip',
    role: { en: 'Full-stack developer', es: 'Desarrollador full-stack' },
    type: { en: 'Microservice platform', es: 'Plataforma de microservicios' },
    title: 'Coinly',
    tagline: {
      en: 'Personal finance, rebuilt as a Spring Boot microservice platform with federated identity and one database per service.',
      es: 'Finanzas personales reconstruidas como una plataforma de microservicios Spring Boot, con identidad federada y una base de datos por servicio.',
    },
    overview: {
      en: [
        'Coinly tracks income, expenses, recurring bills, accounts and savings goals. The interesting part is not the domain but the shape: instead of one monolith it runs as **four independent Spring Boot applications** that find each other through Netflix Eureka and are reached from the outside through a single Spring Cloud Gateway.',
        'Identity is delegated entirely to a self-hosted **Keycloak 26**, chosen over a cloud IdP because the whole stack has to work on a LAN with no internet. The realm — roles, clients, PKCE settings — is versioned as `coinly-realm.json` and imported at boot, so wiping the container and starting again gives byte-identical auth.',
        'Every service owns its own PostgreSQL database and none of them share a schema. Services reference each other **by identifier, never by foreign key**: an `Account` stores the `userSubject` string that matches the `sub` claim of the Keycloak JWT, so the finance service never needs a JPA relation into the user service.',
        'The SPA is React + TypeScript with a design direction of its own ("Quiet Cards"), signing in through a native form that posts straight to Keycloak\'s token endpoint — no OIDC redirect, no PKCE, because `crypto.randomUUID` is not available over plain HTTP on the LAN.',
      ],
      es: [
        'Coinly lleva ingresos, gastos, recibos recurrentes, cuentas y objetivos de ahorro. Lo interesante no es el dominio sino la forma: en lugar de un monolito son **cuatro aplicaciones Spring Boot independientes** que se descubren entre sí con Netflix Eureka y a las que se llega desde fuera por un único Spring Cloud Gateway.',
        'La identidad se delega por completo en un **Keycloak 26** autoalojado, elegido frente a un IdP en la nube porque todo el stack debe funcionar en LAN sin internet. El realm — roles, clientes, ajustes de PKCE — está versionado como `coinly-realm.json` y se importa al arrancar, así que borrar el contenedor y empezar de cero devuelve exactamente la misma autenticación.',
        'Cada servicio es dueño de su propia base de datos PostgreSQL y ninguno comparte esquema. Los servicios se referencian **por identificador, nunca por clave ajena**: una `Account` guarda el string `userSubject` que coincide con el claim `sub` del JWT de Keycloak, de modo que el servicio de finanzas jamás necesita una relación JPA contra el de usuarios.',
        'La SPA es React + TypeScript con dirección de diseño propia ("Quiet Cards") y entra por un formulario nativo que postea directamente al endpoint de token de Keycloak — sin redirección OIDC ni PKCE, porque `crypto.randomUUID` no existe sobre HTTP plano en la LAN.',
      ],
    },
    features: [
      { icon: '◈', en: { title: 'Gateway as the only door', text: 'Spring Cloud Gateway routes to services by Eureka name (lb://) — clients never learn a port.' }, es: { title: 'El gateway como única puerta', text: 'Spring Cloud Gateway enruta a los servicios por nombre de Eureka (lb://) — el cliente nunca conoce un puerto.' } },
      { icon: '◉', en: { title: 'One database per service', text: 'user-service and finance-service each own a Postgres schema inside a single instance.' }, es: { title: 'Una BD por servicio', text: 'user-service y finance-service tienen cada uno su esquema Postgres dentro de una única instancia.' } },
      { icon: '⬡', en: { title: 'Keycloak identity', text: 'OIDC realm with VIEWER / ADMIN roles, provisioned from a versioned realm import.' }, es: { title: 'Identidad con Keycloak', text: 'Realm OIDC con roles VIEWER / ADMIN, aprovisionado desde un import versionado.' } },
      { icon: '▣', en: { title: 'Wallet SPA', text: 'Accounts, transactions, recurring bills and goals, in Spanish and English.' }, es: { title: 'SPA de cartera', text: 'Cuentas, movimientos, recibos recurrentes y objetivos, en español e inglés.' } },
      { icon: '◱', en: { title: 'Money that adds up', text: 'Amounts stored as BigDecimal(19,2) end to end — no floating point anywhere near a balance.' }, es: { title: 'Dinero que cuadra', text: 'Importes en BigDecimal(19,2) de punta a punta — ningún coma flotante cerca de un saldo.' } },
    ],
    arch: ['React SPA', 'Spring Cloud Gateway', 'Eureka discovery', 'user-service', 'finance-service', 'Keycloak', 'PostgreSQL'],
    stack: {
      frontend: ['React 18', 'Vite 5', 'TypeScript', 'React Router 6', 'keycloak-js'],
      backend: ['Java 21', 'Spring Boot 3.5', 'Spring Cloud Gateway', 'Eureka', 'Spring Security OAuth2', 'Spring Data JPA'],
      database: ['PostgreSQL 16'],
      devops: ['Docker Compose', 'Gradle multi-module', 'Keycloak 26'],
    },
    challenges: [
      { en: { title: 'Identity without foreign keys', text: 'Splitting users and finances into separate databases meant giving up referential integrity across the boundary — the JWT `sub` became the join key, and orphan handling moved into the service layer.' }, es: { title: 'Identidad sin claves ajenas', text: 'Separar usuarios y finanzas en bases de datos distintas significó renunciar a la integridad referencial en la frontera — el `sub` del JWT pasó a ser la clave de unión, y la gestión de huérfanos se mudó a la capa de servicio.' } },
      { en: { title: 'Defence in depth', text: 'The gateway validates the JWT and forwards the Authorization header, but user-service and finance-service revalidate it as resource servers — a request that skips the gateway is still rejected.' }, es: { title: 'Defensa en profundidad', text: 'El gateway valida el JWT y reenvía la cabecera Authorization, pero user-service y finance-service la revalidan como resource servers — una petición que se salte el gateway sigue siendo rechazada.' } },
      { en: { title: 'Auth over plain HTTP', text: 'The standard PKCE redirect flow needs a secure context. On a plain-HTTP LAN it simply does not run, so login falls back to a direct grant posted from the SPA to Keycloak, same-origin through the Vite proxy.' }, es: { title: 'Auth sobre HTTP plano', text: 'El flujo estándar de redirección con PKCE necesita un contexto seguro. En una LAN sobre HTTP plano sencillamente no arranca, así que el login recurre a un direct grant que la SPA postea a Keycloak, del mismo origen vía proxy de Vite.' } },
    ],
    links: { live: null, github: null, download: null },
    gallery: [
      { caption: { en: 'Wallet dashboard', es: 'Panel de la cartera' }, render: () => <ImgMock src="/projects/coinly-1.jpg" alt="Coinly wallet dashboard" /> },
      { caption: { en: 'Transactions', es: 'Movimientos' }, render: () => <ImgMock src="/projects/coinly-2.jpg" alt="Coinly transactions" /> },
      { caption: { en: 'Sign-in — the "Quiet Cards" direction', es: 'Login — la dirección "Quiet Cards"' }, render: () => <ImgMock src="/projects/coinly-3.jpg" alt="Coinly sign-in" /> },
    ],
  },
  {
    id: 'sotrixos',
    ...
    year: '2026',
    status: 'active',
    role: { en: 'Full-stack & infrastructure', es: 'Full-stack e infraestructura' },
    type: { en: 'Home automation panel', es: 'Panel domótico' },
    title: 'sotrixOS',
    tagline: {
      en: 'A self-hosted control panel for the whole house — machines, plugs, consoles and a local AI assistant, with nothing leaving the network.',
      es: 'Un panel de control autoalojado para toda la casa — equipos, enchufes, consolas y un asistente de IA local, sin que nada salga de la red.',
    },
    overview: {
      en: [
        'sotrixOS is a control panel built on top of Home Assistant, with its own **FastAPI** backend and a React dashboard covering Home, Machines, System, Assistant and Sensors. It replaces a pile of separate apps with one screen that actually knows what is on.',
        'The assistant chain is **Home Assistant → local rules → LM Studio**. Deterministic aliases resolve the common phrasings without ever reaching a model; anything else is interpreted by an LLM running on the same machine. Voice works the same way, transcribed by Wyoming Whisper and spoken back by Piper — no cloud speech service, no audio leaving the house.',
        'Power control is the part that had to be taken seriously. Two Windows PCs wake by Wake-on-LAN and shut down over OpenSSH, each with **its own key pair, its own pinned host fingerprint and a `ForceCommand`** that limits that key to `status`, `shutdown` and `restart` — a compromised key does not open a shell, and does not reach the other machine.',
        'Sensitive actions are gated: they need an explicit one-shot confirmation, run with a 60-second margin, and are written to a local audit log. Cutting a relay is treated as an emergency operation and is deliberately unavailable to the assistant and to voice; restoring power is chained automatically, because closing a relay cannot take anything down.',
        'Everything is published **only inside the Tailscale tailnet** — no service listens on the LAN address and no port is forwarded from the router.',
      ],
      es: [
        'sotrixOS es un panel de control construido sobre Home Assistant, con backend propio en **FastAPI** y un dashboard React con Inicio, Equipos, Sistema, Asistente y Sensores. Sustituye un montón de apps sueltas por una sola pantalla que de verdad sabe qué está encendido.',
        'La cadena del asistente es **Home Assistant → reglas locales → LM Studio**. Unos alias deterministas resuelven las frases habituales sin llegar nunca a un modelo; el resto lo interpreta un LLM que corre en la misma máquina. La voz funciona igual: la transcribe Wyoming Whisper y la responde Piper — sin servicio de voz en la nube, sin audio que salga de casa.',
        'El control de energía es la parte que había que tomarse en serio. Dos PC Windows se encienden por Wake-on-LAN y se apagan por OpenSSH, cada uno con **su propio par de claves, su propia huella fijada y un `ForceCommand`** que limita esa clave a `status`, `shutdown` y `restart` — una clave comprometida no abre una consola, y no alcanza a la otra máquina.',
        'Las acciones sensibles pasan por un cerrojo: exigen una confirmación explícita de un solo uso, se programan con 60 segundos de margen y quedan en una auditoría local. Cortar un relé se trata como operación de emergencia y a propósito no existe ni para el asistente ni para la voz; devolver la corriente sí se encadena solo, porque cerrar un relé no puede tumbar nada.',
        'Todo se publica **únicamente dentro del tailnet de Tailscale** — ningún servicio escucha en la IP de la LAN y no hay ningún puerto abierto en el router.',
      ],
    },
    features: [
      { icon: '◉', en: { title: 'Local AI assistant', text: 'Natural-language control through LM Studio, with deterministic rules short-circuiting the common commands.' }, es: { title: 'Asistente de IA local', text: 'Control en lenguaje natural vía LM Studio, con reglas deterministas que atajan las órdenes habituales.' } },
      { icon: '◑', en: { title: 'Voice, end to end local', text: 'Wyoming Whisper transcribes and Piper speaks — in Spanish and English, with no cloud round trip.' }, es: { title: 'Voz, local de punta a punta', text: 'Wyoming Whisper transcribe y Piper responde — en español e inglés, sin pasar por la nube.' } },
      { icon: '⚡', en: { title: 'Real power control', text: 'Wake-on-LAN plus restricted SSH for PCs, HDMI-CEC for the TV, MQTT for the PS5 and smart plugs.' }, es: { title: 'Control de energía real', text: 'Wake-on-LAN más SSH restringido para los PC, HDMI-CEC para la tele, MQTT para la PS5 y los enchufes.' } },
      { icon: '⛨', en: { title: 'Confirmation and audit', text: 'Sensitive actions need a one-shot confirmation, run on a 60-second timer and land in a local audit log.' }, es: { title: 'Confirmación y auditoría', text: 'Las acciones sensibles exigen confirmación de un solo uso, corren con 60 segundos de margen y quedan auditadas en local.' } },
      { icon: '✉', en: { title: 'Telegram bot', text: 'The same commands by long polling from a single allowed chat_id, voice notes included.' }, es: { title: 'Bot de Telegram', text: 'Las mismas órdenes por long polling desde un único chat_id autorizado, notas de voz incluidas.' } },
      { icon: '▤', en: { title: 'Live telemetry', text: 'CPU, memory, GPU, temperature and every Docker container, streamed over WebSocket.' }, es: { title: 'Telemetría en directo', text: 'CPU, memoria, GPU, temperatura y todos los contenedores Docker, en directo por WebSocket.' } },
    ],
    arch: ['React dashboard', 'FastAPI + action policy', 'Home Assistant', 'MQTT / Zigbee', 'LM Studio', 'Whisper + Piper', 'Tailscale'],
    stack: {
      frontend: ['React', 'TypeScript', 'Vite', 'lucide-react'],
      backend: ['FastAPI', 'Python', 'Home Assistant', 'LM Studio', 'Wyoming Whisper', 'Piper', 'Telegram Bot API'],
      database: ['SQLite'],
      devops: ['Docker Compose', 'Mosquitto MQTT', 'ESPHome', 'Tailscale', 'Nginx'],
    },
    challenges: [
      { en: { title: 'A TV that lies about its state', text: 'Titan OS keeps its API alive in standby and reports the screen wrong, so `toggle` had no safe direction to move in. The fix was to stop trusting the TV: a smart plug\'s wattage became the source of truth, with a threshold deciding on or off.' }, es: { title: 'Una tele que miente sobre su estado', text: 'Titan OS mantiene su API viva en reposo y devuelve mal el estado de la pantalla, así que `toggle` no tenía hacia dónde conmutar con seguridad. La solución fue dejar de fiarse de la tele: el consumo de un enchufe medidor pasó a ser la fuente de verdad, con un umbral que decide encendido o apagado.' } },
      { en: { title: 'Waits long enough to look broken', text: 'Powering the TV or the PS5 back on can take a minute once the relay has to close first. Without a signal the panel looks frozen and the user presses again — exactly what the backend lock rejects. The pending notice is published *before* the await, not after.' }, es: { title: 'Esperas tan largas que parecen un cuelgue', text: 'Encender la tele o la PS5 puede tardar un minuto cuando antes hay que cerrar el relé. Sin un aviso el panel parece colgado y el usuario vuelve a pulsar — justo lo que rechaza el cerrojo del backend. El aviso se publica *antes* del await, no después.' } },
      { en: { title: 'Which actions the AI may not take', text: 'An LLM interpreting free text cannot be allowed to cut power to the server. Actions are classified as sensitive or not, and the destructive ones exist only as an explicit button in the panel — never as an assistant tool, never as a voice command.' }, es: { title: 'Qué acciones no puede tomar la IA', text: 'A un LLM que interpreta texto libre no se le puede permitir cortarle la corriente al servidor. Las acciones se clasifican como sensibles o no, y las destructivas existen solo como botón explícito en el panel — nunca como herramienta del asistente, nunca como orden de voz.' } },
    ],
    links: { live: null, github: null, download: null },
    gallery: [
      { caption: { en: 'Home — quick view of the house', es: 'Inicio — vista rápida de la casa' }, render: () => <ImgMock src="/projects/sotrixos-1.jpg" alt="sotrixOS home" /> },
      { caption: { en: 'Machines — power control', es: 'Equipos — control de energía' }, render: () => <ImgMock src="/projects/sotrixos-2.jpg" alt="sotrixOS machines" /> },
      { caption: { en: 'Assistant and audit log', es: 'Asistente y auditoría' }, render: () => <ImgMock src="/projects/sotrixos-3.jpg" alt="sotrixOS assistant" /> },
      { caption: { en: 'System telemetry on mobile', es: 'Telemetría del sistema en móvil' }, render: () => <ImgMock src="/projects/sotrixos-4.jpg" alt="sotrixOS system on mobile" /> },
    ],
  },
]
